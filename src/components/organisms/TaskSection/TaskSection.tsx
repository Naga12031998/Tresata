...
export const TaskSection: React.FC<TaskSectionProps> = ({
  ...
  return (
    <div className="task-section">
      <div className="task-section-header" onClick={onToggle}>
        <span className="task-section-title">
          {title} <span className="task-section-count">({tasks.length})</span>
        </span>
        <span className={`task-section-toggle ${isExpanded ? "expanded" : ""}`}>
      ...
      {isExpanded && (
        <div className="task-section-content">
          {tasks.length === 0 ? (
            <div className="task-section-empty">No tasks in this section</div>
          ) : (
            tasks.map((task) => (
              <TaskCard
                key={task.id}
